<template>
  <nav class='navbar navbar-light'>
    <div class='container'>
      <router-link :to="{name:'home'}" class='navbar-brand ng-binding'>Real World</router-link>
      <ul class='nav navbar-nav pull-xs-right'>
        <li class='nav-item'>
          <router-link class='nav-link' :to='{name:"home"}' active-class='active'>Home</router-link>
        </li>
        <template v-if='!userIsLoggedIn'>
          <li class='nav-item'>
            <router-link class='nav-link' :to='{name:"login"}' active-class='active'>Sign in</router-link>
          </li>
          <li class='nav-item'>
            <router-link class='nav-link' :to='{name:"register"}' active-class='active'>Sign up</router-link>
          </li>
        </template>
        <!-- Show this for logged in users -->
        <template v-if='userIsLoggedIn'>
          <li class='nav-item'>
            <router-link class='nav-link' :to='{name:"edit"}' active-class='active'>
              <i class='ion-compose'></i>&nbsp;New Article
            </router-link>
          </li>
          <li class='nav-item'>
            <router-link class='nav-link' :to='{name:"settings"}' active-class='active'>
              <i class='ion-gear-a'></i>&nbsp;Settings
            </router-link>
          </li>

          <li class='nav-item'>
            <router-link class='nav-link' :to='{name:"userProfile", params:{slug:currentUser.username}}'>
              <img class='user-pic' v-if='currentUser.image' :src='currentUser.image' />
              {{ currentUser.username }}
            </router-link>
          </li>
        </template>
      </ul>
    </div>
  </nav>
</template>

<script>
export default {
  name: 'RwHeader',
  computed: {
    userIsLoggedIn() {
      return this.$store.state.auth.userIsLoggedIn
    },
    currentUser() {
      return this.$store.state.auth.currentUser
    }
  }
}
</script>
